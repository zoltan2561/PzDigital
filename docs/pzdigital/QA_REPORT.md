...
## V1.9 egyenes vizuál és természetes szöveg — 2026-09-17

- [x] A hero, kísérőcímke, termékképek és a közös mockupok CSS/GSAP-forgatása, perspektívája és hover-nagyítása a forrásból megszűnt. A főcím/CTA azonnal látható; a képes belépés 8–10 px, legfeljebb 0,6 másodperc. A paletta, főcím, CTA-k és szekciósorrend változatlan.
- [x] A brief szerinti célzott főoldali, SzervizPRO-, FoodShop-, GyrosCity- és kapcsolati szövegcserék elkészültek. Az üres médiadobozok kimaradtak, a funkcióleírások és a későbbi valódi kép megjelenítése megmaradt. A FoodShop valódi GyrosCity-mobilképe álló arányban, teljesképes linkkel szerepel.
- [x] Marketing- és megkereséstesztek: 31 PASS, 319 assertion; Vite build és Pint PASS. Jó beküldés/mentés, hibás mező, rate limit, kontextus és értesítési állapotok ellenőrizve memóriabeli SQLite és Bus/Mail fake mellett. Nem történt valódi SMTP-küldés.
- [x] 320/390/768/1024/1280/1440/1920 px tördelés, 390/1440 px részletes screenshotok; nincs dokumentumszintű vízszintes túlcsordulás, törött kép vagy JS-konzolhiba. Mobilmenü/Escape/fókusz, billentyűzetes GYIK, JS nélküli tartalom és natív/szerveroldali üresmező-validáció ellenőrizve.
- [x] A `motion.mjs` a képeket és szülőiket is vizsgálja: mátrix lineáris tagjai, külön `rotate`, `scale`, `perspective`. A belépési minták és a 390/1440 px normál/reduced-motion alap-, fókusz- és hoverállapotok torzításmentesek a főoldal, terméklista, két termékoldal és GyrosCity oldalán. Layout-eltolás nem minősül forgatásnak. Élő reduced-motion váltás után 0 inline animációs stílus.
- [x] A `qa.mjs` megőrzi a fehér/kék címsor külön sorát; gradient, áttetszőség és pszeudoelem esetén `needs_manual_review`, nem hamis automatikus PASS/FAIL. A normál/hover gombszíneket külön rögzíti. A `final-check.json` kézi vizuális ellenőrzéssel kiegészített, konzervatív CSS-színhatárokat és a végleges mobilkép középre igazítását dokumentálja.
- [x] Záró mobiljavítás: a korábbi 1,48-as galériaarány csak a nem álló képekre érvényes. A betöltött eredeti kép 390 × 844, a 390 px-es nézetben megjelenített kép 305 × 660 px; a teljes kép aránya megmarad. A végső médiaaudit ezt külön `ratioPreserved` adattal ellenőrzi minden vizsgált szélességen, nem csak a keret túlcsordulását figyeli.
- [x] A `reflow320Simulation` 320 px-es reflow-szimuláció, nem valódi 400%-os böngészőzoom. A gyökér 200%-os betűméreténél a törzsszöveg kétszeres, a fluid címek kb. 1,65/1,81-szeresek, a px-alapú gombszöveg változatlan; ezért ez nem teljes szövegnagyítási PASS és nem akadálymentességi tanúsítás.
- [x] Előtte/utána hero és termékpanelek 1440/390 px-en; termékbemutató, FoodShop mobilkép, referencia és űrlapképek; friss `qa-report.json`, `motion-report.json`, `final-check.json` és kb. 7 másodperces időbélyeges animációfelvétel: `storage/app/qa/v1-9`. Ezek helyi, figyelmen kívül hagyott QA-artifactok.
- [x] Közvetlen tulajdonosi kérés szerint a célág `main`, ami az ellenőrzéskor még nem létezett a remote-on. Kiindulás: `04d765c`, korábban feltöltött feature-állapot. Repoellenőrzés: 0 GitHub Actions workflow, üres webhooklista, nincs dokumentált deploy-hook. A GitHub alapértelmezett ágát nem változtatjuk meg; nincs force push vagy külön élesítési művelet.
- [ ] Továbbra is nyitott: végleges adatkezelési/jogi jóváhagyás és éles SMTP/queue ellenőrzés. A kapcsolatoldali indulási megjegyzés megmaradt; a munka nem minősíti élesítésre késznek az űrlapot.
